...
prompt_pure_preprompt_render() {
	...
	if [[ -n $prompt_pure_git_stash ]]; then
		preprompt_parts+=('%F{$prompt_pure_colors[git:stash]}${prompt_pure_git_stash}%f')
	fi
...
prompt_pure_check_git_stash() {
	setopt localoptions noshwordsplit
	local stash stashes=${1:-0}

	(( stashes > 0 )) && stash+=${PURE_GIT_STASH_SYMBOL:-$}

	[[ -n $stash ]] || return
	typeset -g REPLY=$stash
}
...
prompt_pure_async_callback() {
	...
	case $job in
		...
		prompt_pure_async_git_stash)
			local REPLY
			prompt_pure_check_git_stash $output
			if [[ $prompt_pure_git_stash != $REPLY ]]; then
				typeset -g prompt_pure_git_stash=$REPLY
				do_render=1
			fi
			;;
